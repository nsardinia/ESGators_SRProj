...
describe("KalshiPage", () => {
  it("loads the ESG trade plan, market data, and recent orders", async () => {
    ...
    try {
      ...
      expect(await screen.findByText(/demo order controls/i)).toBeInTheDocument()
      expect(await screen.findByText(/current order/i)).toBeInTheDocument()
      expect(await screen.findByRole("button", { name: /place buy order/i })).toBeInTheDocument()
